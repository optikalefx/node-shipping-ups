...
function UPS(args) {
  ...
  function buildShipment(data) {
    ...
    if(data.services && typeof (data.services instanceof Array)) {
      for(var i = 0; i < data.services.length; i++) {
        var s = { 'Service': {
          'Code': data.services[i]
        }};
        shipment['#list'].push(s);
      }
    }
    ...
    for(var i = 0; i < data.packages.length; i++) {
      var p = { 'Package': {
        'PackagingType': {
          'Code': data.packages[i].packaging_type || '02'
        },
        'PackageWeight': {
  ...
  $scope.buildRatesRequest = function(data, options) {
    ...
    var request = {
      'Request': {
        'RequestAction': 'Rate',
        'RequestOption': (data.services && data.services.length > 0) ? 'Rate' : 'Shop'
      },
      'PickupType': {
